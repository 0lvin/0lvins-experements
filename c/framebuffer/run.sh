...
arm-linux-gnueabihf-gcc --pedantic --static ../../fb.c ../../utils.c  -o init

find . | cpio -o -H newc | gzip > ../initrd.img-3.2.0-4-vexpress-new

cd ..

# qemu-system-arm -M vexpress-a9 -kernel vmlinuz-3.2.0-4-vexpress -initrd initrd.img-3.2.0-4-vexpress-new -drive if=sd,file=check_image.qcow2 -append "root=/dev/mmcblk0p2 console=ttyAMA0" -net nic,model=lan9118,netdev=net0 -netdev user,id=net0,hostfwd=tcp::2222-:22 -serial stdio
