/*
  ...
  cssanno.c -- annotate HTML documents with CSS declarations

  Compile:

    gcc cssanno-html.c `pkg-config --cflags libcroco-0.6` `pkg-config  libcroco-0.6 --libs` -o cssanno-html

  Usage:

    cssanno-html file.html file.css

  cssanno takes an HTML document and a CSS style sheet and
...
int main(int argc, char** argv)
{
    ...
    if (!(w->document = htmlParseFile(xmlfile, NULL)))
